...
//   100644 f7e8d9c0b1a2...  169990100 128 src/main.c
//
...
int index_load(Index *index) {
    ...
    while (fgets(line, sizeof(line), f) && index->count < MAX_INDEX_ENTRIES) {
        ...
        if (sscanf(line, "%o %64s %ld %u ", &e->mode, hash_hex, &e->mtime_sec, &e->size) >= 4) {
            char *path_ptr = line;
            // Advance path_ptr to the start of the path (after 4 spaces)
            for (int i = 0; i < 4; i++) {
...
int index_save(const Index *index) {
    ...
    // Sorting the actual index in-place is safer than copying the whole struct
    // Cast away const for sorting purposes
...
int index_add(Index *index, const char *path) {
    ...
    // Fix the fread warning by checking the return value
    size_t read_bytes = fread(data, 1, st.st_size, f);
    fclose(f);
    
    if (read_bytes != (size_t)st.st_size && st.st_size > 0) {
        free(data);
        return -1;
    }

    ObjectID blob_id;
    // Write contents as OBJ_BLOB
